...
class MeanStdMetric(Metric):
    def __init__(self, nfeats=166, max_len=600):
        ...
        self.add_state("sums", default=torch.zeros(self.max_len,self.nfeats), dist_reduce_fx="sum")
        self.add_state(
            "sum_of_squares", default=torch.zeros(self.max_len, self.nfeats), dist_reduce_fx="sum"
        )
    ...
    def update(self, feature_tensors: torch.Tensor, num_frames: int) -> None:
        ...
        if current_len < self.max_len:
            padding = torch.zeros((self.max_len - current_len, self.nfeats), device=feature_tensors.device)
